    ...
    <div style={{minHeight:'100vh', background:'var(--ink)', display:'grid', placeItems:'center', padding:'28px 16px'}} onClick={()=>navigate('/dashboard')}>
      <div className="panel" style={{width:'100%', maxWidth:520, overflow:'hidden'}} onClick={e=>e.stopPropagation()}>
        <div style={{padding:'18px 18px 14px', display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid var(--line)'}}>
          ...
          <span className="mono row-meta" style={{marginLeft:'auto', cursor:'pointer'}} onClick={()=>navigator.clipboard?.writeText(friend.username)}>@{friend.username}</span>
        </div>
        <div style={{padding:22, textAlign:'center'}}>
          ...
          <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:8, flexWrap:'wrap'}}><h2 className="display" style={{fontSize:22}}>{friend.alias||friend.username}</h2>{friend.is_buddy && <Star size={15} fill="#FFB224" color="#FFB224"/>}<span className={`mono status-pill`} style={{fontSize:12, padding:'4px 10px', borderRadius:999, border:'1px solid var(--line)', color: friend.isOnline?'var(--success)':'var(--text-3)'}}>{friend.isOnline?'online':'offline'}</span></div>
          {friend.alias && <div className="mono row-meta" style={{marginTop:4}}>@{friend.username}</div>}
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, padding:'0 18px 18px'}}>
          <div style={{padding:14, borderRadius:12, background:'var(--surface-raised)', border:'1px solid var(--line)', textAlign:'center'}}><Calendar size={14} style={{margin:'0 auto 6px', display:'block', color:'var(--text-3)'}}/><div className="stat-label">Connected</div><div className="stat-value" style={{marginTop:4}}>{friend.created_at? format(new Date(friend.created_at*1000),'MMM yyyy'): 'Recently'}</div></div>
          <div style={{padding:14, borderRadius:12, background:'var(--surface-raised)', border:'1px solid var(--line)', textAlign:'center'}}><Clock size={14} style={{margin:'0 auto 6px', display:'block', color:'var(--text-3)'}}/><div className="stat-label">Talk time</div><div className="stat-value" style={{marginTop:4}}>{talk()}</div></div>
        </div>
        <div style={{padding:'0 18px 18px', display:'flex', flexDirection:'column', gap:10}}>
          <label className="label">Alias</label>
          <div className="field__wrap"><Hash size={12}/><input value={editAlias} onChange={e=>setEditAlias(e.target.value)} placeholder="Custom name" /><button onClick={updateAlias} disabled={saving||!editAlias.trim()} className="btn btn--secondary btn--compact" style={{whiteSpace:'nowrap'}}>{saving?'Saving…':'Save'}</button></div>
